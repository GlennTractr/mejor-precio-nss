...
const getTypesenseConfig = () => {
  ...
  const apiKey = process.env.NEXT_PUBLIC_TYPESENSE_ADMIN_API_KEY || process.env.TYPESENSE_ADMIN_API_KEY;
  ...
  if (!apiKey) {
    throw new Error('Typesense API key is not configured. Please set NEXT_PUBLIC_TYPESENSE_ADMIN_API_KEY or TYPESENSE_ADMIN_API_KEY');
  }
